...
const STEPS = [
    {
        ...
        desc: 'Have a natural conversation with our Vapi-powered concierge. No forms, no friction — just speak.',
        color: 'sky',
    },
    {
        icon: Zap,
        title: 'AI Extraction',
        desc: 'Gemini parses your interview into a structured setup profile — use-cases, channels, persona traits.',
        color: 'violet',
    },
    {
        icon: FileText,
        title: 'Setup Guide',
        desc: 'A personalized OPENCLAW_ENGINE_SETUP_GUIDE.md, ready to download and deploy immediately.',
        color: 'sky',
    },
];

const FEATURES = [
    { icon: Volume2, label: 'Native Voice', detail: 'Real-time duplex audio via Vapi' },
    { icon: Shield, label: 'Privacy First', detail: 'Runs entirely on your infrastructure' },
    { icon: Sparkles, label: 'Smart Defaults', detail: 'Registry-aware skill matching' },
];

export default function EasyClawLanding({ onStart }) {
    return (
        <div className="noise-overlay min-h-screen bg-[var(--ec-bg)] grid-bg relative overflow-x-hidden">
            {/* Ambient glow */}
            ...
            {/* Hero */}
            <section className="relative z-10 max-w-5xl mx-auto px-6 md:px-12 pt-20 md:pt-32 pb-20 text-center">
                ...
                <h1 className="text-6xl md:text-8xl font-bold tracking-tight leading-[1.05] mb-4">
                ...
                <p className="text-lg md:text-2xl text-slate-300 font-medium mb-3">
                    Voice-powered OpenClaw setup
                </p>

                <p className="text-base md:text-lg text-slate-500 max-w-xl mx-auto mb-12 leading-relaxed">
                    Just talk to our AI concierge — get a personalized engine setup guide in seconds.
                </p>
                ...
                <div className="mt-16 md:mt-20 relative mx-auto w-fit">
                    <div className="absolute inset-0 rounded-full bg-sky-500/10 blur-[60px] scale-125" />
                    <div className="relative animate-float">
                        <div className="absolute -inset-6 rounded-full border border-sky-500/15 animate-pulse-ring" />
                        <div className="absolute -inset-3 rounded-full border border-sky-500/10" />
                        <img
                            src="/agent_listening_avatar.png"
                            alt="EasyClaw Concierge"
                            className="w-44 h-44 md:w-56 md:h-56 rounded-full object-cover border-2 border-sky-500/25 shadow-[0_0_60px_rgba(56,189,248,0.15)]"
                    ...
                    <div className="flex flex-col md:flex-row items-center gap-10 md:gap-14">
                        <div className="flex-1">
                            ...
                            <p className="text-sm text-slate-400 leading-relaxed mb-6">
                                Agent 1 conducts a voice interview via Vapi, capturing your
                                use-cases, preferred channels, and persona traits.
                                Agent 2 maps your profile against the OpenClaw registry and generates
                                a fully personalized engine setup guide — all in under 60 seconds.
                            </p>
                        ...
                        <div className="flex-1 w-full">
                            <div className="rounded-xl border border-white/[0.06] bg-[var(--ec-surface)] p-5 font-mono text-xs space-y-2.5">
                                <PipeRow label="VAPI" detail="Voice Interview (Agent 1)" c="sky" />
                                <PipeArrow />
                                <PipeRow label="FORMATTER" detail="Gemini Transcript Parser" c="violet" />
                                <PipeArrow />
                                <PipeRow label="ADK RUNNER" detail="Output Agent (Agent 2)" c="sky" />
                                <PipeArrow />
                                <PipeRow label="GUIDE" detail="OPENCLAW_ENGINE_SETUP_GUIDE.md" c="emerald" />
                            </div>
                        </div>
                    </div>
                </div>
...
function PipeRow({ label, detail, c }) {
    const cls = {
        sky: 'border-sky-500/25 text-sky-400 bg-sky-500/[0.06]',
        violet: 'border-violet-500/25 text-violet-400 bg-violet-500/[0.06]',
        emerald: 'border-emerald-500/25 text-emerald-400 bg-emerald-500/[0.06]',
    };
    return (
        <div className={`flex items-center gap-3 px-3 py-2.5 rounded-lg border ${cls[c]}`}>
            <span className="font-semibold tracking-wider text-[10px] uppercase w-24 shrink-0">{label}</span>
...
function PipeArrow() {
    return (
        <div className="flex justify-center">
            <div className="w-px h-3 bg-gradient-to-b from-sky-500/30 to-transparent" />
        </div>
    );
}
