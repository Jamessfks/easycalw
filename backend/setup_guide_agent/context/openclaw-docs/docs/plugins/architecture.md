...
Use `openclaw plugins inspect ` to see a plugin's shape and capability
...
When you run `openclaw doctor` or `openclaw plugins inspect `, you may see
one of these labels:
...
* Bundled extension internals remain private. External plugins should use only
  ...
  import `extensions//src/*` from core or from another extension.
* Repo entry point split:
  `extensions//api.js` is the helper/types barrel,
