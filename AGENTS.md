...
├── setup_guide_agent/
│   ├── __init__.py
│   ├── agent.py              # Setup Guide Creation Agent (Claude Agent SDK)
...
│   └── components/
│       ├── AgentPresence.jsx  # 3 avatar PNGs (listening/thinking/talking) + animated mic circle
...
backend/setup_guide_agent/context/  # Knowledge base — read-only for agent
```
...
- Build two-panel layout: `InterviewView.jsx`
  - Left: `AgentPresence.jsx` — swap between listening/thinking/talking PNGs based on Vapi `speech-start`/`speech-end` events + animated mic circle
...
- Uses **Claude Agent SDK** with read-only access to `context/` knowledge base
...
- Modify files in `backend/setup_guide_agent/context/`
...
*Aligned with OpenClaw Concierge architecture v4.3 — 2026-03-24.*
